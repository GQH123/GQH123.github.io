...
void init(){
    ...
    for (int i = 1; i < bound; i++) phi[i] = (phi[i] + phi[i - 1]) % M;
}
map<LL, LL> save;
LL dyh(LL x){
    if (x < bound) return phi[x];
    if (save.count(x)) return save[x];
    LL ans = (x & 1) ? (x % M) * (((x + 1) >> 1) % M) : ((x >> 1) % M) * ((x + 1) % M);
    ans %= M;
    for (LL i = 2; i <= x; i++){
        LL next = x / (x / i);
...
void work(LL n){
    ...
    for (LL i = 1; i <= n; i++){
        LL next = n / (n / i);
        ans = (ans + dyh(n / i) * ((next - i + 1) % M) % M * ((next + i) % M) % M * ((M + 1) >> 1) % M) % M;
        i = next;
    }
    LL rec = (n & 1) ? (n % M) * (((n + 1) >> 1) % M) : ((n >> 1) % M) * ((n + 1) % M);
    rec %= M;
    ans = (M + ans - rec) % M;
    ans = ans * 2 % M;
    ans = (ans + rec) % M;
